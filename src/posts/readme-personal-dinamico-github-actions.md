...
<ul>
  {{#posts}}
    <li><a href="{{{id}}}"><strong>{{title}}</strong></a><br/><em>{{summary}}</em></li>
  {{/posts}}
</ul>
...
Desde entonces han cambiado un poco las cosas y desde Node 14.8 (en Agosto de 202) podemos usar `await` en un script sin necesidad de una función así que vamos a escribir una función que nos traiga el contenido de `readme.mustache`:
...
```js
const blogPosts = await fetch('https://antonio.laguna.es/feed/feed.json').then(res => res.json());
...
¡La de cosas que hemos hecho en 9 líneas de código!
...
Espero que te haya servido y, si te animas a hacer algo dinámico, ¡avísame por [Twitter](https://twitter.com/ant_laguna) o por [correo](https://antonio.laguna.es/sobre-mi/) para que vea cómo lo has hecho!